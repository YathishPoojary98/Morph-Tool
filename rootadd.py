...
with open("exceptions.txt","r") as f1:
    exceptions = f1.readlines()
    exceptions = [i.strip() for i in exceptions]
    exception_roots = [i.split(",")[0] for i in exceptions]
    exception_suffs = [i.split(",")[-1] for i in exceptions]

with open("exceptional_words.txt","r") as f2:
    ...
    for suff in suffixes:
            ...
            if w.endswith(suff):
                ...
                if pre=='':
                    return (word,removed_suffs)
                f = 1
                if search_in_dict(pre) and root_return == -1:
...
with open("rootsandsuff.txt","w") as f:
    for i in range(len(data)):
        ...
        if flag==0:
            ...
            if ((pos=='N_NNV' or pos=='N__NNV') or cat=='pn') and to_match=='vu':
                suff = "ವು+ಉ"
            elif root in exception_roots and to_match=='vu' and exception_suffs=='vu':
                suff = "ವು+ಉ"
            ...
            if (pos.endswith("NNV") or cat=='adj') and (root_wx not in exception_words):
                for i in nnv_adds:
                    if root.endswith(i):
                        root = root[:-2]
                        if suff==0:
            ...
            if root_wx.endswith("aweV") and (root_wx not in exception_words):
            ...
            suff_replace = {'ಅಂ+ತ+ಎ':'ಅಂತೆ',"ಇ+ಕೆ":"ಇಕೆ",'ಇ+ದ':'ಇದ','ವ+ಅರ':'ಅವರು','ವಳಿ+ಕೆ':'ವಳಿಕೆ','ವ+ವ+ಅನು':'ವ+ಅವನು','+ಅ+ಬಹುದು':'+ಬಹುದು','ಓ+ಉದು':'ಓದು'}
            for a,b in suff_replace.items():
                if str(suff).endswith(a):
                    print("yes")
                if (str(suff).endswith(a)) or (f"{a}+" in str(suff)):
                    print("Here in suuf_replace")
